...
(function() {
	if (window.CanvasRenderingContext2D) {
		const H = 'hearts', D = 'diamonds', S = 'spades', C = 'clubs', J = 'joker', N = 'nine', R = 'crown', T = 200, O = 150;
		var symbolPath = {
			...
			return args[0] + n * args[2] / T;
		}, ay = function(n) {
			var args = arguments.callee.caller.arguments;
			return args[1] + n * args[2] / T;
		}, as = function(n) {
			return n * arguments.callee.caller.arguments[2] / T;
		}, pc = function(size) {
			var canvas = document.createElement('canvas');
			canvas.height = size = size || T;
			canvas.width = size * 3 / 4;
			return canvas;
		}
		//for reduce minified file size
		symbolPath[H] = 'M100,30C60,7 0,7 0,76C0,131 100,190 100,190C100,190 200,131 200,76C200,7 140,7 100,30z';
		symbolPath[D] = 'M184,100C152,120,120,160,100,200C80,160,48,120,16,100C48,80,80,40,100,0C120,40,152,80,184,100z';
		symbolPath[S] = 'M200,120C200,168,144,176,116,156C116,180,116,188,128,200C112,196,88,196,72,200C84,188,84,180,84,156C56,176,0,168,0,120C0,72,60,36,100,0C140,36,200,72,200,120z';
		symbolPath[C] = 'M80,200C92,184,92,160,92,136C76,180,0,176,0,124C0,80,40,76,68,88C80,92,80,88,72,84C44,64,40,0,100,0C160,0,156,64,128,84C120,88,120,92,132,88C160,76,200,80,200,124C200,176,124,180,108,136C108,160,108,184,120,200C100,196,100,196,80,200z';
		symbolPath[R] = 'M44,60 C45,56 -3,33 0,70 C2,107 39,146 48,150 C57,154 12,107 12,77 C12,45 43,65 44,60 M37,65 C31,64 20,60 19,81 C19,100 63,158 65,149 C65,139 33,102 37,65 M86,56 C87,52 38,28 40,66 C43,103 69,141 78,148 C86,155 54,102 54,71 C54,39 86,60 86,56 M82,65 C77,64 59,54 59,74 C60,95 82,146 84,138 C86,132 78,102 82,65 M154,60 C153,56 203,33 200,70 C197,107 159,146 151,150 C142,154 187,107 187,77 C187,45 155,65 154,60 M161,65 C167,64 179,60 180,81 C181,100 137,158 135,149 C134,139 165,102 161,65 M113,56 C112,52 161,28 158,66 C155,103 130,141 122,148 C114,155 145,102 145,71 C145,39 114,60 113,56 M117,65 C123,64 141,54 141,74 C140,95 118,146 116,138 C114,132 121,102 117,65z';
		symbolPath[N] = 'M157,89C159,188 80,211 16,196L23,160C62,172 100,167 107,119C93,127 83,133 62,132C28,133 0,113 0,70C0,25 37,0 78,0C137,0 157,41 157,89M105,56C100,42 92,34 77,33C59,33 49,49 49,69C52,101 83,104 107,95C107,82 108,66 105,56z';
		symbolPath[J] = 'M141,0L181,0C168,55 161,150 129,183C91,219 15,198 21,141L60,137C58,157 62,166 81,166C102,165 110,143 115,118M6,378C6,306 53,256 119,256C197,256 213,346 187,398C164,438 130,458 88,459C39,458 7,422 6,378M47,377C49,406 67,425 93,425C168,423 182,292 115,290C69,294 47,338 47,377M0,714L42,518L84,518L66,601L159,518L215,518L124,595L191,714L144,714L94,621L55,654L42,714M8,973L50,777L200,777L193,809L85,809L75,854L180,854L173,887L68,887L56,940L173,940L166,973M43,1231L1,1231L44,1035L133,1035C170,1037 197,1051 198,1087C195,1127 169,1143 136,1148C158,1171 171,1206 182,1231L137,1231C116,1182 112,1150 60,1150M67,1121C96,1121 155,1126 156,1087C151,1061 100,1068 78,1068z';
		...
		CanvasRenderingContext2D.prototype.drawPokerCard = function(x, y, size, suit, point) {
			...
			this.drawEmptyCard(ax(0), ay(0), as(T));
			...
			if (size >= 100) {
				if (point !== J) {
					...
					this.fillPokerSymbol(ax(11), ay(10), as(18), J);
					this.fillPokerSymbol(ax(139), ay(190), as(-18), J);
					if (suit === H || suit === D) {
				...
				if (point !== J) {
					...
					this.fillPokerSymbol(ax(11), ay(10), as(22), J);
					if (suit === H || suit === D) {
		...
		CanvasRenderingContext2D.prototype.svgCurve = function(x, y, size, svgPath) {
			var relativeX, relativeY, pathNumber, pathArray, svgpathArray, ax = function(n) {
				return ( relativeX = x + n * size / T);
			}, ay = function(n) {
				return ( relativeY = y + n * size / T);
			};
		...
		CanvasRenderingContext2D.prototype.drawEmptyCard = function(x, y, size, startColor, endColor) {
			...
			fillLinGrad = this.createLinearGradient(ax(5), ay(5), ax(55), ay(T));
			fillLinGrad.addColorStop(0, '#ffffff');
			fillLinGrad.addColorStop(1, '#e0e0e0');

			this.fillStyle = fillLinGrad;
			this.fillRoundRect(ax(0), ay(0), as(O), as(T), as(16));
			this.strokeStyle = '#666';
			this.strokeRoundRect(ax(0), ay(0), as(O), as(T), as(16));
		};
